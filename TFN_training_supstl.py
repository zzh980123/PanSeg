...
def main():
    ...
    model = TFN_huge.TransFlowNet(args.model_name.lower(), device, args, in_channels=1)
    ...
    for epoch in range(1, max_epochs):
        ...
        for step, batch_data in train_bar:
            ...
            loss = loss_function(outputs, labels) + loss_function(trans_f, trans_f_label) + 0.1 * sloss_function(s_normal, s) + 0.01 * sloss_function(xy_normal, xy)
            loss.backward()
            optimizer.step()
