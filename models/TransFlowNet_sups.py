...
class TransForward(nn.Module):
    def __init__(self, in_channels=1, hidden_dim=16, out_channels=4, max_scaling=4):
        ...
        self.upSample1 = common.Up(hidden_dim * 2, hidden_dim, False)
        self.upSample2 = common.Up(hidden_dim, out_channels, False)
    ...
    def forward(self, x):
        ...
        u1 = self.upSample1(d2, d1)  # (b, 16, 256, 256)
        params = self.upSample2(u1, x)  # (b, 4, 512, 512)
        ...
        return trans_f, params, s_normal
...
class TransFlowNet(nn.Module):
    ...
    def forward(self, x):
        trans_f, params, s_normal = self.trans_forward(x)
        feature = self.model(trans_f)
        trans_b, output = self.trans_back(feature, params)
        return trans_f, feature, output, s_normal
