...
class TransForward(nn.Module):
    def __init__(self, in_channels=1, hidden_dim=16, out_channels=2, max_scaling=4):
        ...
        self.downSample1 = common.Down(hidden_dim, hidden_dim * 2)
        self.downSample2 = common.Down(hidden_dim * 2, hidden_dim * 4)
        self.downSample3 = common.Down(hidden_dim * 4, hidden_dim * 8)
        self.downSample4 = common.Down(hidden_dim * 8, hidden_dim * 16)
        self.downSample5 = common.Down(hidden_dim * 16, hidden_dim * 32)

        self.upSample1 = common.Up(hidden_dim * 32, hidden_dim * 16, False)
        self.upSample2 = common.Up(hidden_dim * 16, hidden_dim * 8, False)
        self.upSample3 = common.Up(hidden_dim * 8, hidden_dim * 4, False)
        self.upSample4 = common.Up(hidden_dim * 4, hidden_dim * 2, False)
        self.upSample5 = common.Up(hidden_dim * 2, hidden_dim, False)
    ...
    def forward(self, x, label):
        ...
        u0 = self.upSample1(d5, d4)
        u1 = self.upSample2(u0, d3)
        u2 = self.upSample3(u1, d2)
        u3 = self.upSample4(u2, d1)
        u4 = self.upSample5(u3, x)
        features = self.conv_reduce(u4)  # (b, 2, 512, 512)          c:0 coarse seg        c:1 learned scale size
