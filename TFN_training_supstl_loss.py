...
import models.TransFlowNet_huge as TFN_hugev1
...
def main():
    ...
    model = TFN_stlv2_5.TransFlowNet(args.model_name.lower(), device, args, in_channels=1)
    ...
    for epoch in range(1, max_epochs):
        ...
        for step, batch_data in train_bar:
            ...
            loss1 = 0.1 * sloss_function(s_normal, s) + 0.01 * sloss_function(xy_normal, xy)
            loss2 = loss_function(outputs, labels) + loss_function(trans_f, trans_f_label) + 0.1 * sloss_function(s_normal, s) + 0.01 * sloss_function(xy_normal, xy)
            loss = loss1 if epoch < 30 else loss2
            loss.backward()
            optimizer.step()
        ...
        if epoch >= 10 and epoch % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for step, val_data in enumerate(val_loader, 1):
                    ...
                    # val_forward, val_forward_label, val_hidden_feature, val_outputs, s_normal, xy_normal = sliding_window_inference(
